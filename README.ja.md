...
- 画像から画像の幅と高さは Qwen Image Edit Runtime へ実際に渡されます。指定比率が入力画像と異なる場合、入力画像を等比率で縮小し、端の延長で目標キャンバスを満たしてから生成するため、元画像全体を保持できます。`128 × 192` も使用できますが、低すぎる解像度では細部と構図の安定性が低下するため、`512 × 768` 以上を推奨します。
