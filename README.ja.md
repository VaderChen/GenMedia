...
テキストから画像と画像から画像のボタンを分け、生成結果を選択しても次の画像を生成できるようにしました。狭いウィンドウではボタンが折り返されます。起動時はルートパッケージの 4 製品を個別にビルド・検査し、ビルド成功後に実行ファイルが見つからない問題を修正しました。スクリプト 12 件、Web UI 6 件のテストと、4 言語の WebKit レイアウト検査が成功しました。
...
LoRA 変換は重みを 1 MiB 単位でコピーします。Worker のログは差分読み取りし、正常終了直前の大量ログも処理します。ルートパッケージ全体のビルドと **163 件の Swift テスト**が成功しました。合成 LoRA 測定と検証範囲は[性能記録](docs/PERFORMANCE_CHANGES.md)（繁体字中国語）を参照してください。すべてのモデルやハードウェアでの動作確認を意味するものではありません。
...
モデルセンターに **Qwen-Image 2.1 MLX 4-bit** を追加しました。同じ重みでテキストから画像と実験的な単一画像編集を利用できます。内蔵 Swift／MLX Worker が RGBA 画像を出力し、Python は不要です。512×512 の画像生成と 256×256 の編集を実測済みです。以前の 512×512 編集で見られた粒状感は、精度修正後に再検証していないため未解決として扱います。手順と制限は [Qwen-Image 2.1 ガイド](docs/QWEN_IMAGE_21.md)（繁体字中国語）を参照してください。

## プレビュー

![GenMedia 操作デモ GIF](images/operation-demo.gif)

操作ガイド：プロンプトと出力設定 → 動画・音楽設定 → モデル検索 → シンプル MV ワークフローの作成。現在の WebUI とデモデータを使用した録画で、モデル推論は実行していません。画面の言語は繁体字中国語です。
...
`build.command` は Release 実行ファイルと標準の `GenMedia.app` を `dist/` に生成します。App には WebUI リソース、MLX Metal ランタイム、MCP サーバー、および統一メディア互換レイヤーとして LGPL 動的版 `ffmpeg`／`ffprobe` が含まれます。初回の App bundle ビルド時に `build.command` がソースを自動ダウンロードし、内蔵 FFmpeg を構築します。FFmpeg の手動準備や `pkg-config` のインストールは不要です。
...
プロジェクトが ExFAT 上にある場合、署名する App の出力先は内部 APFS に指定してください。例：`GENIMAGE_DIST_DIR="$HOME/Downloads/GenMedia-dist" ./build.command`。AppleDouble メタデータによる入れ子 bundle の署名エラーを避けられます。完成した DMG は外付けディスクへコピーできます。
...
- ワークスペースのフィルムストリップには画像読み込みボタンがあり、Finder から PNG、JPEG、WebP、GIF、TIFF、HEIC、HEIF を 1 枚以上ドロップできます。音楽生成中はメディアソースの混在を防ぐため画像読み込みを無効にします。テキストから画像のボタンは常に表示し、画像を選択すると画像から画像のボタンも表示します。それぞれのプロファイルに従って有効になり、結果の選択を解除せずに次の画像を生成できます。
- 画像と動画の比率項目はドロップダウンになっています。画像から画像では入力画像を選択した場合だけ「元の解像度」を表示し、入力画像のサイズを Runtime が扱える 16 の倍数へ変換します。
- 以下の条件画像キャンバスと 1024² 生成面積の規則は Qwen Image Edit 2511 に適用します。Qwen-Image 2.1 は指定サイズで生成します。[専用ガイド](docs/QWEN_IMAGE_21.md)を参照してください。
- 画像から画像の幅と高さは Qwen Image Edit 2511 Runtime へ実際に渡されます。指定比率が入力画像と異なる場合、入力画像は自身の解像度を保ったまま端の延長で出力比率のキャンバスに配置してから生成するため、元画像全体を保持できます。
