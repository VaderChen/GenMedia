...
## 최근 수정 및 검증 (2026-09-22)

텍스트→이미지와 이미지→이미지 버튼을 분리하여 생성 결과를 선택한 뒤에도 계속 생성할 수 있습니다. 좁은 창에서는 버튼이 줄바꿈됩니다. 실행 스크립트는 루트 패키지의 배포 제품 4개를 각각 빌드하고 확인하여 빌드 성공 후 실행 파일이 없는 문제를 수정했습니다. 스크립트 테스트 12개, Web UI 테스트 6개와 4개 언어의 WebKit 레이아웃 검사를 통과했습니다.
...
LoRA 변환은 가중치를 1 MiB 단위로 복사합니다. Worker 로그는 증분으로 읽으며 정상 종료 직전의 대량 로그도 처리합니다. 루트 패키지 전체 빌드와 **Swift 테스트 163개**를 통과했습니다. 합성 LoRA 측정 및 검증 범위는 [성능 기록](docs/PERFORMANCE_CHANGES.md)(번체 중국어)을 참조하세요. 모든 모델과 하드웨어 조합을 검증했다는 의미는 아닙니다.
...
모델 센터에 **Qwen-Image 2.1 MLX 4-bit**를 추가했습니다. 같은 가중치로 텍스트→이미지와 실험적인 단일 이미지 편집을 제공합니다. 내장 Swift/MLX Worker가 RGBA를 출력하며 Python은 필요하지 않습니다. 512×512 이미지 생성과 256×256 편집을 실측했습니다. 이전 512×512 편집의 입자 문제는 정밀도 수정 후 재검증하지 않아 미해결로 남겨 둡니다. 설정과 제한은 [Qwen-Image 2.1 안내](docs/QWEN_IMAGE_21.md)(번체 중국어)를 참고하세요.

## 미리보기

![GenMedia 인터페이스 사용 안내 GIF](images/operation-demo.gif)

사용 안내: 프롬프트 및 출력 설정 → 동영상·음악 설정 → 모델 검색 → 간단한 MV 워크플로 만들기. 현재 WebUI와 데모 데이터로 녹화했으며 모델 추론은 실행하지 않았습니다. 화면 언어는 번체 중국어입니다.
...
`build.command`는 Release 실행 파일과 표준 `GenMedia.app`를 `dist/`에 생성합니다. App에는 WebUI 리소스, MLX Metal 런타임, MCP 서버와 통합 미디어 호환 계층인 LGPL 동적 `ffmpeg`/`ffprobe`가 포함됩니다. 첫 App bundle 빌드 시 `build.command`가 소스를 자동으로 다운로드하고 내장 FFmpeg 배포판을 준비합니다. FFmpeg를 수동으로 준비하거나 `pkg-config`를 설치할 필요가 없습니다.
...
프로젝트가 ExFAT에 있다면 서명할 App의 출력 위치를 내부 APFS로 지정하세요. 예: `GENIMAGE_DIST_DIR="$HOME/Downloads/GenMedia-dist" ./build.command`. AppleDouble 메타데이터가 중첩 bundle 서명에 영향을 주는 문제를 피할 수 있습니다. 완성한 DMG는 외장 디스크로 복사할 수 있습니다.
...
- 작업 공간 필름스트립에 이미지 가져오기 버튼이 있으며 Finder에서 PNG, JPEG, WebP, GIF, TIFF, HEIC, HEIF 파일을 하나 이상 끌어 놓을 수 있습니다. 음악 생성 중에는 미디어 소스가 섞이지 않도록 이미지 가져오기를 비활성화합니다. 텍스트→이미지 버튼은 항상 표시하며, 이미지를 선택하면 별도의 이미지→이미지 버튼을 표시합니다. 각 버튼은 해당 프로필에 따라 활성화되므로 생성 결과의 선택을 해제하지 않고 다시 생성할 수 있습니다.
- 이미지와 비디오 비율 항목은 드롭다운으로 제공됩니다. 이미지→이미지에서는 원본 이미지를 선택한 뒤에만 `원본 해상도`가 표시되며, 원본 크기를 Runtime에서 사용할 수 있는 16의 배수로 변환합니다.
- 아래 조건 이미지 캔버스 및 1024² 생성 면적 규칙은 Qwen Image Edit 2511에 적용됩니다. Qwen-Image 2.1은 지정한 크기로 생성합니다. [전용 안내](docs/QWEN_IMAGE_21.md)를 참고하세요.
- 이미지→이미지의 너비와 높이는 Qwen Image Edit 2511 Runtime에 실제로 전달됩니다. 지정한 비율이 원본과 다르면 원본은 자체 해상도를 유지한 채 가장자리 확장으로 출력 비율 캔버스에 배치된 후 생성되므로 원본 전체 내용을 보존합니다.
