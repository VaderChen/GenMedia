...
`upstream/FL2VA/processor` within the H3 model directory
...
Full text encoder check (set `H3_MODEL_DIR` to the installed H3 model directory):

```sh
RuntimeSupport/MiniMaxH3Worker/.build/out/Products/Release/GenImageMiniMaxH3Worker text-check \
  --text-encoder "${H3_MODEL_DIR:?Set H3_MODEL_DIR}/text_encoders/qwen3vl_32b_minimax_h3-Q4_K_M.gguf" \
  --tokenizer "${H3_MODEL_DIR}/upstream/FL2VA/processor" \
