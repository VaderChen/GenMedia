...
- 自動媒體清理僅限 MediaCache，保留來源檔與仍被引用的代理；啟動時除了資產 ID，也依實際檔案路徑判斷是否為孤兒，避免重複匯入的快取被誤刪。刪除工作區時會回收無參照的播放代理。
- 同一模型的下載、修復與移除依序執行；暫停後重新下載會等待舊任務及續傳資訊寫入完成。切換模型目錄前先停止下載並等待清理，進行中的移除會完成後才掃描。
- 模型移除改在背景執行，避免大型目錄清理卡住介面；生成工作與移除互斥，移除期間不接受同一模型的新操作。
- 修正連線建立前的取消遺失，以及取消返回後仍寫入續傳檔的競爭。下載與模型硬連結重用先在目標磁碟暫存，再原子替換；失敗時保留既有檔案，HTTP 錯誤內容最多讀取 2 KiB。
- 輸出目錄同步更新至圖片與字幕服務，避免設定顯示新路徑但下一個工作仍寫入舊目錄；進行中的工作固定使用開始時的路徑。
- 模型啟動／目錄切換掃描、下載後驗證與修復驗證移至背景；只採用最新掃描結果，失敗時保留原目錄與清單。LoRA 清單使用獨立背景掃描，保留自訂 Profile 與已保存的選取設定。
- LoRA 相容性檢查只讀取有上限的 safetensors JSON 標頭；格式轉換以 1 MiB 分塊複製權重，完成後才原子發布快取，並可在區塊間取消。256 MiB 合成轉換測試的峰值 RSS 由約 521 MiB 降至 11.6 MiB。
- 修正常駐 Worker 不讀 stdin 時，大型請求會卡在同步寫入而無法取消；改用非阻塞寫入、取消檢查與寫入逾時。
- 修正下載大小差異比較的運算優先順序，避免大小不變仍反覆更新模型清單；已進入驗證階段的任務不再被延遲進度回呼改回下載中。切換 Profile 時重用背景系統資源讀值。
- 修正批次生成圖片共用檔名而互相覆寫；輸出名稱加入 UUID，取消或失敗時清除該批次的未完成檔案。
- 修正媒體預覽停止後仍呼叫 WebKit 的問題，逐塊等待主執行緒交付，避免整段影片累積在回呼佇列；空白 Range 標頭會回報錯誤。
...
- 系統資源、任務進度與下載進度使用獨立的輕量 Web Bridge 訊息；避免每秒重新建立資產、讀取字幕、查詢憑證與序列化整份模型清單。工作區存檔移至背景佇列並合併連續修改，結束 App 前同步完成存檔。
- 圖片格線與底片列改用最大 384 px 的延遲載入縮圖，原圖預覽保持完整解析度；縮圖快取限制為 16 MiB／256 張。
- Runtime 日誌只讀取新增的行；Upscale 重用 Vision 模型包裝並逐 tile 寫入單一畫布；H3 MP4 輸出逐幀轉換，避免保留整段影片的 CPU 圖片陣列。
- 修正 H3 影片寫入使用 BGRA 記憶體卻標為 ARGB 的色彩錯誤；同時修正 Worker 的根套件名稱，使其可由 GenImage 原始碼目錄建置。

## 1.26.0902 — 2026-09-02

- MiniMax H3 影片生成新增低解析度品質警告：技術最低尺寸為 `64×64`，寬高向下對齊至 `32` 的倍數；短邊低於 `512 px` 時由 App 顯示可取消的確認對話框，但不阻擋使用者繼續生成。
- H3 低解析度警告顯示目前尺寸、實際對齊後尺寸與原生品質參考，避免 `256×160` 等過低設定在未確認的情況下直接生成模糊或色塊化影片。
- 保留既有 H3 Worker 的含音訊影片寫入修正，避免長影片在 `AVAssetWriter` 背壓期間停滯；H3 仍處於測試階段，不保證所有模型變體、輸出或硬體配置的正確性。
