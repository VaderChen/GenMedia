...
MLX metallib files are versioned by the MLX Swift runtime. The main application, Qwen Image Edit, MiniMax Music 3, and LTX use `RuntimeSupport/mlx-swift-0.31.6.metallib`; the isolated Z-Image worker uses `RuntimeSupport/mlx-swift-0.30.6-zimage.metallib`. `build.command` reads each `Package.resolved`, verifies compatible versions, and copies the matching library into the release executable directory. Model license review and 16/24/32 GB stress testing remain release requirements.
